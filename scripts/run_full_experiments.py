...
def print_completeness_report(
    ...
    completed = {
        (r.seed, r.protocol, r.model) for r in results if r.status in ("success", "skipped")
    }
    missing = []
    ...
    for protocol in expected_protocols:
        protocol_results = [
            r for r in results if r.protocol == protocol and r.status in ("success", "skipped")
        ]
        ...
        table.add_row(
            protocol,
            f"{seeds_done}/{len(expected_seeds)}",
            ", ".join(sorted(models_done)) or "-",
            status,
        )
            ...
            for protocol in protocols:
                for model in models:
                    ...
                    if result.status == "success":
                        console.print(
                            f"  [green]✓[/green] seed={seed}, {protocol}/{model} ({result.duration_s:.1f}s)"
                        )
                    elif result.status == "skipped":
                        console.print(
                            f"  [yellow]○[/yellow] seed={seed}, {protocol}/{model} (skipped - exists)"
                        )
                    else:
                        console.print(
                            f"  [red]✗[/red] seed={seed}, {protocol}/{model}: {result.error}"
                        )
