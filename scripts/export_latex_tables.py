...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Export evaluation results to LaTeX table format")
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        "--all-protocols", action="store_true", help="Generate tables for all protocol directories"
    )
    parser.add_argument(
        "--output", type=Path, default=None, help="Output file (default: print to stdout)"
    )
    ...
    for protocol_dir in protocol_dirs:
        ...
        output_lines.append(
            generate_main_results_table(tables_csv, caption=f"Main Results - {protocol_name}")
        )

        # Budget table
        budget_csv = protocol_dir / "budget_tables.csv"
        output_lines.append(
            generate_budget_table(budget_csv, caption=f"Budget Evaluation - {protocol_name}")
        )

        # Ablation table
        output_lines.append(
            generate_ablation_table(protocol_dir, caption=f"Context Ablation - {protocol_name}")
        )
