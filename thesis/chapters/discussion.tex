...
\subsection{Interpretation of Baseline Model Performance}
\label{subsec:discussion_baselines}

The baseline evaluation (Table~\ref{tab:main-results}) shows that even relatively simple machine learning models can achieve strong performance on the synthetic ToolShield dataset, while purely rule-based approaches remain substantially weaker.

\subsubsection{Why TF-IDF + Logistic Regression performs strongly}
...
\subsubsection{Why heuristic approaches are weaker}
...
\subsubsection{Transformer-based models and the role of context}
...
The \texttt{S\_tool\_holdout} protocol can produce single-class training splits depending on the tool--label distribution. In such cases, supervised training is not meaningful and affected model--protocol combinations must be skipped. This is an important practical constraint for synthetic dataset design: tool-level holdout protocols require careful balancing to avoid degenerate training distributions.

\subsection{Truncation Bias as a Catastrophic Failure Mode}
\label{subsec:discussion_truncation_bias}

The most significant result of this thesis is that truncation strategy can dominate model performance when tool schemas become large. The long-schema stress test provides evidence that naive truncation is not merely a minor preprocessing detail, but a critical robustness factor.

\subsubsection{Why truncation occurs in context-augmented tool-use inputs}
...
\subsubsection{Empirical evidence of prompt removal}
...
The prompt retention collapse is further visualized in Figure~\ref{fig:prompt-retention-quartiles} and Figure~\ref{fig:prompt-retention-deciles}, which show that naive truncation consistently eliminates prompt tokens across schema length bins.

\subsubsection{Why the failure is catastrophic}
...
\subsubsection{Why truncation bias is not visible in the standard dataset}
...
\subsection{Robustness vs.\ Latency Tradeoff}
...
\subsection{Implications for Real-World LLM Tool-Use Systems}
...
\subsubsection{Schema length as a deployment reality}
...
\subsubsection{Guard models must prioritize the user prompt}
...
\subsubsection{Operating-point evaluation matters}
...
\subsubsection{Schema inflation method}
...
\subsubsection{Limited model diversity}
...
\subsection{Reproducibility and Credibility of Experimental Evidence}
...
\subsection{Summary of Contributions and Remaining Open Problems}
