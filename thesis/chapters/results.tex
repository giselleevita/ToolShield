...
\section{Baseline Model Performance on the Standard Dataset}
\label{subsec:baseline_standard}

Table~\ref{tab:main-results} summarizes performance on the standard dataset. Overall, nearly all models achieve strong discrimination, with transformer-based approaches reaching near-ceiling classification under this benchmark.

\subsection{\texttt{S\_random} Results}
...
\begin{itemize}
    ...
    \item \texttt{transformer} (text-only) achieves ROC-AUC $=0.9977$ and PR-AUC $=0.9982$; it matches the context-aware variants at FPR@TPR90 $=0.000$ but shows a small penalty at the strictest operating point (FPR@TPR95 $=0.040$ vs.\ $0.000$ for the context-aware models).
...
The context-aware transformer achieves near-perfect ranking performance on this split. This suggests that, under the synthetic data distribution, context features (tool schema, tool description, and role framing) provide sufficient information to separate benign from malicious prompts.
...
\subsection{\texttt{S\_attack\_holdout} Results}
...
\section{Attack Success Rate (ASR) Reduction}
\label{subsec:asr_reduction}

We report ASR reduction at two operating points: TPR90 and TPR95 (Table~\ref{tab:asr-results}). ASR reduction measures how effectively the detector blocks malicious prompts when deployed as a binary gate before tool execution. A budget-constrained threshold-selection evaluation (thresholds chosen on the validation split to satisfy fixed FPR budgets) was also implemented; however, because the transformer models produce strongly saturated score distributions, validation-selected thresholds do not transfer reliably to the test split and the resulting operating points are degenerate. These results are therefore not reported as tables; making budget-based threshold transfer robust under score saturation is left to future work.

\subsection{\texttt{S\_random}}
...
\subsection{\texttt{S\_attack\_holdout}}
...
Inference latency was measured in warm mode. All reported values are wall-clock times for scoring a \emph{fixed batch of 100 test samples in a single call} (not per-request latencies); dividing by the batch size gives an approximate per-sample cost (e.g., ${\sim}\,40$\,ms per sample for \texttt{context\_transformer\_keep\_prompt}). The values are comparable across models measured on the same batch, but should not be read directly as per-request serving latency. Table~\ref{tab:latency-results} summarizes P50 and P95 latencies.

On the standard dataset, the following latency trends are observed:

\begin{itemize}
    \item \texttt{tfidf\_lr} is among the fastest models with P50 latency $=2.42$\,ms.
    \item Transformer-based models exhibit the highest latencies: \texttt{context\_transformer\_keep\_prompt} has P50 latency $=3997.70$\,ms, while the text-only \texttt{transformer} (shorter inputs, no context fields) runs at $737.43$\,ms.
    \item \texttt{context\_transformer\_naive} has P50 latency $=2469.62$\,ms, lower than \texttt{keep\_prompt} due to the simpler single-pass tokenization.
\end{itemize}

Thus, prompt-preserving truncation introduces additional runtime overhead, roughly increasing P50 latency from ${\sim}\,2.5$\,s to ${\sim}\,4.0$\,s per 100-sample batch in this environment. This is expected, since the \texttt{keep\_prompt} strategy performs two-pass tokenization and explicit budget allocation.

\section{Truncation Ablation on the Standard Dataset}
...
On the standard dataset, both strategies yield nearly identical predictive performance (Table~\ref{tab:main-results}). For example, under \texttt{S\_attack\_holdout}, ROC-AUC is $0.9987$ for \texttt{keep\_prompt} and $0.9982$ for \texttt{naive}.
...
\subsection{Long-schema stress test (production-scale approximation)}
\label{subsec:longschema_stress_test}

To evaluate truncation behaviour under more production-scale tool schemas in this synthetic benchmark, we constructed a long-schema dataset variant where tool schemas were inflated to approximately 4\,000 characters, corresponding to approximately 840 schema tokens on average (Table~\ref{tab:longschema-stress}).
...
The naive model performs close to random chance (ROC-AUC $\approx 0.5$). Additionally, the operating-point metrics show that it cannot achieve a usable detection threshold: FPR@TPR90 increases to $0.92$ and FPR@TPR95 increases to $0.95$. No validation-selected threshold satisfies the FPR budgets while maintaining meaningful detection.

In contrast, \texttt{keep\_prompt} maintains high detection performance under the evaluated conditions with FPR@TPR90 $=0.000$ and FPR@TPR95 $=0.010$, consistent with the standard dataset results.
...
The \texttt{keep\_prompt} strategy avoids this failure by enforcing a minimum prompt token budget, ensuring that detection remains effective under the evaluated long-schema conditions even when tool schemas dominate the context.
...
\begin{itemize}
    ...
    \item Under production-scale schemas in the stress test (${\sim}\,4\,000$ characters, ${\sim}\,840$ tokens), \texttt{naive} truncation causes $100\%$ prompt truncation and prompt retention ratio collapses to $0.0$ (Table~\ref{tab:longschema-stress}, Figure~\ref{fig:prompt-retention-quartiles}).
    ...
    \item Within this synthetic benchmark, prompt-preserving truncation is therefore a necessary design choice for effective prompt injection detection under long-schema conditions.
\end{itemize}
