...
Inference latency is measured using timed prediction runs over a fixed batch of 100 test samples in a single call, repeated for $n_{\text{runs}}{=}10$ timing runs after warmup. ToolShield reports P50 and P95 percentiles of these batch times under warm-start conditions. Because each timing covers a 100-sample batch rather than an individual request, the values support relative comparison between models but do not directly estimate per-request serving latency.
