...
This chapter describes the methodology used to design, implement, and evaluate \textit{ToolShield}, a prompt injection detection system for tool-using LLM agents. The approach combines controlled synthetic dataset generation, multiple model baselines, protocol-driven evaluation splits, and reproducibility-focused experiment automation.
...
\subsubsection{Benign Prompt Generation}
...
\subsubsection{Attack Prompt Generation}
...
\subsubsection{Dataset Output and Manifest}
...
\subsection{Tool Schema Structure}
...
\subsection{Prompt Injection Families}
\label{subsec:attack_families}

A key objective of ToolShield is to measure robustness to diverse injection strategies. Therefore, attacks are grouped into four families (AF1--AF4), each representing a distinct manipulation style. The family structure is intended to approximate the diversity observed in real prompt injection attacks described in prior work~\cite{CITATION_PROMPT_INJECTION_SURVEY}. % TODO: Replace with actual citation key

\subsubsection{AF1: Direct Instruction Override}
...
\subsubsection{AF2: Context Extraction and Data Exfiltration}
...
\subsubsection{AF3: Tool Manipulation and Parameter Smuggling}
...
\subsubsection{AF4: Indirect or Obfuscated Attacks}
...
\subsubsection{S\_random: Stratified Random Split}
...
\subsubsection{S\_attack\_holdout: Attack Family Holdout}
...
\subsubsection{S\_tool\_holdout: Tool Holdout}
...
\subsubsection{Scored Heuristic Baseline}
...
\subsubsection{Transformer (Text-Only)}
...
\subsubsection{Context Transformer}
...
\subsection{Prompt-Preserving Truncation Strategy}
...
\subsubsection{Naive Truncation}

In the naive strategy, context and prompt are concatenated into one string and passed to the tokenizer with a fixed \texttt{max\_length}. The tokenizer applies standard truncation, typically removing tokens from the tail.

This can result in complete loss of the prompt when tool schemas are long.

\subsubsection{Keep-Prompt Truncation}
...
\subsubsection{Attack Success Rate (ASR) Reduction}
...
\subsubsection{Budget-Based Threshold Selection}
...
\subsection{Determinism and Reproducibility Measures}
...
\subsubsection{Manifest and Environment Freeze}
...
\subsubsection{Automated Verification Scripts}
...
\subsubsection{Controlled Experiment Automation}
