...
\subsection{Controlled experimental setting}
...
\subsection{Template artifacts and distribution simplifications}
...
\subsection{Risk of overfitting to synthetic prompt style}
...
\subsection{Seed propagation defect and single-seed neural results}
The experiment runner was intended to train each model under three seeds ($0, 1, 2$) and to report the mean and standard deviation across seeds. A configuration-key mismatch --- the runner supplied the seed under the key \texttt{random\_state}, whereas the transformer and context-transformer models read the key \texttt{seed} --- caused these neural models to fall back to their default seed regardless of the requested value. Consequently, the three ``seeds'' produced identical transformer models, and the cross-seed standard deviation reported for these models is zero by construction rather than as evidence of stability. Only the deterministic \texttt{tfidf\_lr} and heuristic baselines are legitimately invariant across seeds. The defect has since been corrected (the runner now passes the \texttt{seed} key, and the models accept either key), but the neural results reported in this thesis should be interpreted as single-seed point estimates; establishing genuine multi-seed variance requires re-running the suite under the corrected configuration and is left as a reproducibility follow-up. This limitation does not affect the qualitative finding of a holdout generalization gap, which is present within each individual run, but the reported variances for the neural models must not be read as robustness across random initialisations.
...
\subsection{Tool schema realism and application diversity}
The benchmark models tool schemas and tool descriptions in a simplified and controlled form. Tool schemas used in large organizations may contain nested JSON structures, long enumerations, multiple parameter groups, and extensive documentation text. Additionally, the semantic meaning of schemas varies across domains (finance, healthcare, internal IT systems), which may influence how prompt injection manifests.

The long-schema stress test approximates production-scale schemas by inflating schema length, but the inflation procedure may not capture the semantic structure of real schemas. Therefore, the observed truncation bias results demonstrate a valid failure mode, but the exact threshold where truncation becomes harmful may differ in real systems depending on schema formatting and tokenization characteristics.

\subsection{Attacker creativity and adaptive strategies}
...
\subsection{Multilingual and cross-cultural prompt injection}
...
\subsection{Multi-turn agents and tool chaining}
...
\subsection{ROC-AUC and PR-AUC versus operational security}
...
\subsection{Limitations of ASR reduction as a security metric}
...
\subsection{Threshold selection and budget constraints}
...
\section{Reproducibility and Reliability Validity}
...
\subsection{Deterministic splits and controlled seeds}
...
\subsection{Environment freeze and experiment manifests}
...
\subsection{Verification scripts and split hygiene checks}
...
\subsection{Residual nondeterminism in ML frameworks}
